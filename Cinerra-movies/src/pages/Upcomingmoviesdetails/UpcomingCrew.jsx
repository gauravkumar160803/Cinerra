...
export default function UpcomingCrew({ upcoming }) {
    const crewMembers = upcoming?.crew || [];
    const cardsPerPage = 6;
    const [startIndex, setStartIndex] = useState(0);

    const visibleCrew = crewMembers.slice(startIndex, startIndex + cardsPerPage);

    const showNext = () => {
        if (startIndex + cardsPerPage < crewMembers.length) {
    ...
    const isNextDisabled = startIndex + cardsPerPage >= crewMembers.length;
        ...
        <section className="py-5 px-2 mx-4 md:mx-20 bg-transparent w-full border-b border-gray-300 max-w-5xl">
            ...
            <div className="max-w-5xl">
                ...
                <div className="relative flex items-center">
                    ...
                    {crewMembers.length > cardsPerPage && (
                        <button
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-x-4 gap-y-6 w-full py-2">
                        {/* FIXED: Using _id, id, or index fallback for unique React keys */}
